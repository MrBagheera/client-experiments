...
public class JSONObject : IEnumerable<KeyValuePair<string, JSONObject>>
{
#if POOLING
    ...
    public void AddFieldIfNotEmpty(string name, JSONObject obj)
    {
    ...
    {
        return (T)Enum.Parse(typeof(T), GetField(name).str ?? defaultValue.ToString());
    }
...
#pragma warning restore 219
	 ...
	 */
	void Stringify(int depth, StringBuilder builder, bool pretty = false, int maxStringLength = -1) {	//Convert the JSONObject into a string
		//Profiler.BeginSample("JSONprint");
		if(depth++ > MAX_DEPTH) {
			Debug.Log("reached max depth!");
			return;
		...
		switch(type) {
			...
			case Type.STRING:
		        ...
		        {
                    var escapedStr = str.Replace("\\", "\\\\").Replace("\"", "\\\"");
                    builder.AppendFormat("\"{0}\"",
		                maxStringLength < 0 || escapedStr.Length <= maxStringLength ? escapedStr : escapedStr.Substring(0, maxStringLength) + "...");
		        }
		        break;
			case Type.NUMBER:
#if USEFLOAT
...
#endif
							builder.AppendFormat("\"{0}\":", key);
							obj.Stringify(depth, builder, pretty, maxStringLength);
							builder.Append(",");
#if(PRETTY)
							if(pretty)
								builder.Append("\n");
						...
						if(list[i]) {
							...
							if(pretty)
								for(int j = 0; j < depth; j++)
									builder.Append("\t"); //for a bit more readability
#endif
							list[i].Stringify(depth, builder, pretty, maxStringLength);
							builder.Append(",");
#if(PRETTY)
							if(pretty)
								builder.Append("\n"); //for a bit more readability
    ...
    public IEnumerator<KeyValuePair<string, JSONObject>> GetEnumerator()
    {
        switch (type)
        {
            case Type.OBJECT:
                for (int i = 0; i < list.Count; i++)
                {
                    yield return new KeyValuePair<string, JSONObject>(keys[i], list[i]);
                }
                break;
            case Type.ARRAY:
                for (int i = 0; i < list.Count; i++)
                {
                    yield return new KeyValuePair<string, JSONObject>(i.ToString(), list[i]);
                }
                break;
            default:
                throw new ArgumentException("JSONObject of type " + type + " cannot be enumerated!");
        }
    }
    ...
    {
        StringBuilder builder = new StringBuilder();
        Stringify(0, builder, false, 20);
        return builder.ToString();
    }

    #region MurMurHash

    private const uint StringSeed = 0xf7ca7fd2;
    private const uint SymmetricSeed = 0xb592f7ae;
    private const uint TraversableSeed = 0xe73a8b15;
    ...
    {
        hash = MixLast(hash, data);
        hash = RotLeft(hash, 13);
        return hash * 5 + 0xe6546b64;
    }

    private static uint MixLast(uint hash, uint data)
    {
        data *= 0xcc9e2d51;
        data = RotLeft(data, 15);
        data *= 0x1b873593;
        return hash ^ data;
    }

    private static uint RotLeft(uint val, int rot)
    {
        return (val << rot) | (val >> (32 - rot));
    }

    private static uint FinalizeHash(uint hash, uint n)
    {
        return Avalanche(hash ^ n);
    }
    ...
    private uint StringHash(string s)
    {
        var hash = StringSeed;
        var i = 0;
        while (i + 1 < s.Length)
        {
            var data = ((uint)s[i] << 16) + s[i + 1];
            hash = Mix(hash, data);
            i += 2;
        }
        if (i < s.Length)
        {
            hash = MixLast(hash, s[i]);
        }
        return FinalizeHash(hash, (uint)s.Length);
    }

    private uint OrderedHash()
    {
        uint n = 0;
        uint hash = SymmetricSeed;
        for (int i = 0; i < list.Count; i++)
        {
            hash = Mix(hash, list[i].Hash());
            n++;
        }
        return FinalizeHash(hash, n);
    }

    private uint UnorderedHash()
    {
        uint a = 0;
        uint b = 0;
        uint n = 0;
        ...
        {
            var elem = list[i];
            var h = StringHash(keys[i]) ^ elem.Hash();
            a += h;
            b ^= h;
            if (h != 0)
            {
